...
func (client *CenterClient) AddPlayer(player *Player) error {
	...
	resp, err := client.Call("add player", string(b))
...
func (client *CenterClient) RemovePlayer(name string) error {
	ret, _ := client.Call("remove player", name)
	if ret.Code == "200" {
		return nil
	}
...
func (client *CenterClient) ListPlayer(params string) (ps []*Player, err error) {
	resp, _ := client.Call("list player", params)
	if resp.Code != "200" {
	...
	if err != nil {
		fmt.Println("List Player Unmarshal failed!")
	}
	return
}
